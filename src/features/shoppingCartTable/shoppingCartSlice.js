...
export const shoppingCartSlice = createSlice({
  ...
  reducers: {
    ...
    allBooksRemovedFromCart: (state, action) => {
      const item = state.shoppingCart.cartItems.find(({ISBN}) => ISBN === action.payload);
      return updateOrder(state, action.payload, -item.count);
    },
  },
});

export const { bookAddedToCart, bookRemovedFromCart, allBooksRemovedFromCart } = shoppingCartSlice.actions;
